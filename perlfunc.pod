...
if any execute bit is set in the mode.  Scripts run by the superuser
may thus need to do a stat FILEHANDLE to determine the
actual mode of the file, or temporarily set their effective uid to
something else.
...
--:
Если используется ACL, существует прагма L<C<filetest>|filetest>, которая
может выдавать более точные результаты, чем голый L<C<stat>|/stat FILEHANDLE>
режим битов.
--:note
