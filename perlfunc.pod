...
--
Функции в этом разделе могут служить термами в выражении.  Они делятся на две
основные категории: списковые операторы и именованные унарные операторы.  Эти
отличия в их относительном приоритере, следующие после запятой. (см.  таблицу
приоритетов в L<perlop>). Списковые операторы принимают более одного аргумента,
тогда как унарные операторы никогда не принимают более одного аргумента.  Таким
образом, запятая завершает аргумент унарного оператора, но лишь отделяет
аргументы спискового оператора.  Унарный оператор обычно обеспечивает скалярный
контекст для его аргументов, в то время как оператор списка может обеспечить
как скалярный, так и списковый контекст для своих аргументов.  Если это так,
скалярные аргументы на первом месте, за которыми следует списковый аргумент,
который может быть только одним аргументом.  Например,
L<C<splice>|/splice ARRAY,OFFSET,LENGTH,LIST> имеет три скалярных аргумента
и список, в то время как L<C<gethostbyname>|/gethostbyname NAME>
имеет четыре скалярных аргумента.
...
--
В описании синтаксиса, который следует, списковые операторы, которые ожидают
список (и обеспечивают списковый контекст для элементов списка) показаны с LIST
как аргумент. Так список может состоять из любой комбинации скалярных
аргументов или списка значений, список значений будет включен в список как если
каждый индивидуальный элемент был интерполирован в этот момент в списке,
формируя больше одномерных списковых значений. Запятые должны отделять
литеральные элементы LIST.
--
В приведенных ниже описаниях синтаксиса перечислены операторы, которые ожидают
список(и предоставляют списковый контекст для элементов списка), показаны с с
аргументом LIST.  Такой список может состоять из любой комбинации скалярных
аргументов или <%значений списка||списка значений%>; значения списка будут
включены в список, как если бы каждый отдельный элемент был интерполирован в
этом месте списка, формируя более длинное одномерное значение списка.  Запятые
должны разделять литеральные элементы LIST.
...
--:old
Любая функция в списке ниже может использоваться как со скобками, так и без них
вокруг своих аргументов. (Описание синтаксиса опущены скобки). Если вы
используете скобки, простой, но иногда неожиданные правила заключается в
следующем: Это выглядит как функция, поэтому она является функцией, и приоритет
не имеет значения. В противном случае, оператор списка или унарный оператор, и
приоритет имеет значение. Пробелы между функцией и левой скобкой не в счет, так
что иногда вы должны быть осторожны:
--:g
Любая функция из приведенного ниже списка может использоваться как с, так и без
круглые скобки вокруг его аргументов. (Синтаксические описания опускают
круглые скобки.) Если вы используете круглые скобки, просто, но иногда
Удивительное правило: это I <выглядит> как функция, поэтому I <is> a
функция и приоритет не имеют значения. В противном случае это список
оператора или унарного оператора, и приоритет имеет значение. Пробелы
между функцией и левой скобкой не учитывается, поэтому иногда
вам нужно быть осторожным:
--:
Любая функция из приведенного ниже списка может использоваться как с круглыми
скобками, так и без них вокруг своих аргументов. (Синтаксические описания
опускают круглые скобки.) Если используются скобки, простое, но иногда
неожиданное правило заключается в следующем: Это выглядит как функция, поэтому
является функцией, и приоритет не имеет значения.  В противном случае, это
оператор списка или унарный оператор, и приоритет имеет значение.  Пробелы
между функцией и левой скобкой не учитываются, поэтому иногда вам нужно быть
осторожным:
...
--:old
Если вы запускаете Perl с переключателем -w, он может предупредить вас об этом.
Например, в третьей строке выше, может воспроизвести:
--:g
Если вы запустите Perl с помощью pragma предупреждений, он может предупредить
вы об этом. Например, третья строка выше дает:
--:
Если запустить Perl с прагмой L<C<use warnings>|warnings>, она может предупредить
вас об этом.
...
--:old
Некоторые функции совсем не принимают аргументы, и поэтому работают как не
унарные, так и не списковые операторы. К ним относятся такие функции, как time
и endpwent. Например, time+86_400 всегда означает time() + 86_400.
--:g
Некоторые функции не принимают никаких аргументов и поэтому не работают ни
унарные или список операторов.
--:
Некоторые функции не принимают никаких аргументов и поэтому ведут себя
не как унарные или списковые операторы.
...
empty list.
--:old
Для функций, которые можно использовать в любом в скалярном или списковом
контексте, неудачные операции как правило, указаны в скалярном контексте
возвратом неопределенного значения, и в списковом контексте возвратом пустого
списка.
--:g
Для функций, которые могут использоваться в скалярном или контекстном
контексте, неабортный отказ обычно указывается в скалярном контексте
посредством возвращая неопределенное значение и в контексте списка, возвращая
пустой список.
--
Для функций, которые могут вызываться как в скалярном так и в списковом
контексте, неудачные операции обычно определяются возвратом неопределенного
значения в скалярном контексте и пустого списка в списковом контексте.
...
X<context>
--:old
Запомните следующее важное правило: Существует правило, которое не относит
поведение выражения в списковом контексте в его поведению в скалярном
контексте, или наоборот. Это может сделать две абсолютно разные вещи. Каждый
оператор и функция решает, какой вид значения наиболее уместно вернуть в
скалярном контексте. Некоторые операторы возвращают длину списка, который был
бы возвращен в списковом контексте. Некоторые операторы возвращают первое
значение списка. Некоторые операторы возвращают последнее значение в списке.
Некоторые операторы возвращают число успешных операций. Вообще, они делают то,
что вы хотите, если вы не хотите консистенции.
--:g
Помните следующее важное правило: нет правила, которое связывает поведение
выражения в контексте списка к его поведению в скалярном контекст или наоборот.
Это может сделать две совершенно разные вещи.  Каждый оператор и функция
решает, какое значение будет больше всего для возврата в скалярном контексте.
Некоторые операторы возвращают длина списка, которая была бы возвращена в
контексте списка. Некоторые операторы возвращают первое значение в списке.
Некоторые операторы возвращают последнее значение в списке. Некоторые операторы
возвращают количество успешных операции. В общем, они делают то, что вы хотите,
если не хотите консистенция.
--:
Помните следующее важное правило: нет правила, которое связывает поведение
выражения в контексте списка к его поведению в скалярном контексте или
наоборот.  Это может сделать две совершенно разные вещи.  Каждый оператор и
функция решает, какого рода значение наиболее уместно вернуть в скалярном
контексте. Некоторые операторы возвращают длину списка, который был бы
возвращен в списковом контексте.  Некоторые операторы возвращают первое
значение списка. Некоторые операторы возвращают последнее значение в списке.
Некоторые операторы возвращают число успешных операций.  Вообще, они делают то,
что вы хотите, если не хотите консистенции.
--:note
Вместо слова 'консистенция' использовать 'логичность' возможно было бы уместнее.
...
--:old
Именованный массив в скалярном контексте весьма отличается от того, что бы на
первый взгляд кажется быть список в скалярном контексте. Вы не можете получить
список как (1,2,3) в быть в скалярном контексте, потому что компилятор знает[не
знает] контекст во время компиляции. Он будет генерировать скалярный оператор
запятой там, а не версию конструкции списка запятой. Нужно начать с того, что
это означает, что она никогда не список
--:g
Именованный массив в скалярном контексте сильно отличается от того, что было бы
первый взгляд представляется списком в скалярном контексте. Вы не можете
получить список например 1,2,3, в скалярном контексте, поскольку компилятор
знает контекст во время компиляции. Он будет генерировать скалярный оператор
запятой там, а не список конкатенации версии запятой. Это означает, что это
никогда не было списком для начала.
--:
Именованный массив в скалярном контексте весьма отличается от того, чем
представлялся бы на первый взгляд список в скалярном контексте. Нельзя получить
список C<(1,2,3)>, находясь в скалярном контексте, потому что компилятор
знает контекст во время компиляции. В этом месте будет порождаться скалярный
оператор запятой, вместо конструкции запятой объединения в список.
Что означает, что это изначально не было списком.
--:note
подобрать более подходящий вариант перевода фразы
"list concatenation version of the comma"
...
accidentally.
--:old
В общем, функции в Perl, которые служат как оболочки для системных вызовов
("syscalls") с тем же самым именем (например L<chown(2)>, L<fork(2)>,
L<closedir(2)>, и.т.д) возвращают истину, когда они успешно завершены и undef в
противном случае, как это обычно упоминается в описании ниже. Это отличается от
интерфейса C, который возвращает -1 при неудаче[при ошибке]. Исключениями из
этого правила являются wain, waitpid, и syscall. Системные вызовы также
устанавливают специальную переменную $! в случае возникновения ошибки. Другие
функции нет, за исключением случайностей.
--:g
--:
В общем, функции в Perl, которые служат как оболочки для системных вызовов
("syscalls") с тем же самым именем (например L<chown(2)>, L<fork(2)>,
L<closedir(2)>, и.т.д) возвращают истину, когда они успешно завершены и undef в
противном случае, как это обычно упоминается в описании ниже.  Это отличается
от интерфейса C, который возвращает C<-1> при ошибке. Исключениями из этого
правила являются L<C<wait>|/wait>, L<C<waitpid>|/waitpid PID,FLAGS>, и
L<C<syscall>|/syscall NUMBER, LIST>.  Системные вызовы также устанавливают
специальную переменную L<C<$!>|perlvar/$!> в случае возникновения ошибки.
Другие функции нет, за исключением непредумышленных случаев.
...
it defines.
--:old
Модули расширения могут также подключить в анализатор Perl для определения
новых видов keyword-headed выражений. Они могут выглядеть как функции, но могут
также выглядеть совершенно иначе. Синтаксис следующих ключевых слов полностью
определяется расширением. Если вы разработчик, см для механизма. Если вы
используете такой модуль, см. документацию модуля для подробной информации о
синтаксисе, который он определяет.
--:g
Модули расширения также могут подключаться к парсеру Perl для определения новых
виды ключевого слова. Они могут выглядеть как функции, но также могут выглядеть
совершенно иначе. Синтаксис, следующий за ключевым словом полностью
определяется расширением. Если вы являетесь разработчиком, см.
L<perlapi/PL_keyword_plugin> для механизма. Если вы используете такие модуль,
см. документацию модуля для подробностей синтаксиса, который он определяет.
--:
Модули расширения также могут подключаться к парсеру Perl для определения новых
видов выражений по ключевым словам. Они могут выглядеть как функции, но могут
также выглядеть совершенно иначе. Синтаксис, следующий за ключевым словом
полностью определяется расширением.  Если вы являетесь разработчиком, см.
L<perlapi/PL_keyword_plugin> для механизма. Если вы используете такой модуль,
см. документацию модуля для подробностей синтаксиса, который он определяет.
--:notes
Найти более подходящий вариант для словосочетания "keyword-headed expression"

=head2 Perl Functions by Category
X<function>
--:
=head2 Функции Perl по категориям
X<function>
...
than one place.
--:old
Вот функции Perl (в том числе вещи, которые выглядят, как функции, похожи
на[такие как] некотоыре ключевые слова и именованные операторы) по категориям.
Некоторые функции появляются более чем в одном месте.
--:g
Вот функции Perl (включая вещи, которые выглядят как
функции, как некоторые ключевые слова и названные операторы)
по категориям. Некоторые функции появляются в более
чем одно место.
--:
Вот функции Perl (включая элементы, которые выглядят как функции, как некоторые
ключевые слова и именованные операторы) по категориям. Некоторые функции
появляются в более чем одном месте.
...
--:
L<C<fc>|/fc EXPR> доступна только если включена поддержка
L<C<"fc"> feature|feature/функции 'fc'> или если она предваряется
префиксом C<CORE::>.  L<C<"fc"> feature|feature/Функция 'fc'>
включена автоматически если объявить C<use v5.16> (или выше) в
текущей области видимости.
--:notes
Найти более подходящий перевод для feature.
Например как "функциональное средство"/"свойство системы"(multitran)
...
X<regular expression> X<regex> X<regexp>
--:g
=item Регулярные выражения и соответствие шаблонов
X<regular expression> X<regex> X<regexp>
...
X<array>
--:old
=item Обработка массивов
--:g
=item Функции для реальных @ARRAY
X<array>
--:
=item Обработка массивов
X<array>
...
X<list>
--:old
=item Обработка списков
--:g
=item Функции для данных списка
X<list>
--:
=item Функции для обработки списков
...
X<hash>
--:old
=item Обработка хэшей
X<hash>
--:
=item Функции для реальных хэшей
X<hash>
...
--:
L<C<say>|/say FILEHANDLE LIST> доступна только если включена поддержка
L<C<"say"> feature|feature/функции 'say'>  или если она предваряется
префиксом C<CORE::>.
L<C<"say"> feature|feature/Функция 'say'> включена автоматически
при объявлении в текущей области видимости C<use v5.10> (или выше).

=item Functions for fixed-length data or records
--:old
=item Функции для данных фиксированной длины и записи
--:g
=item Функции для данных или записей фиксированной длины
--:
=item Функции для данных или записей фиксированной длины
...
--:old
=item Функции для указателей файлов, файлов и директорий
X<file> X<filehandle> X<directory> X<pipe> X<link> X<symlink>
--:g
=item Функции для дескрипторов файлов, файлов или каталогов
X<file> X<filehandle> X<directory> X<pipe> X<link> X<symlink>
--:
=item Функции для дескрипторов файлов, файлов или каталогов
X<file> X<filehandle> X<directory> X<pipe> X<link> X<symlink>
...
X<control flow>
--:old
=item Управление последовательностью выполнения команд Perl
X<control flow>
--:g
=item Ключевые слова, связанные с потоком управления вашей программой Perl
X<control flow>
--:
=item Ключевые слова, связанные с потоком управления программой на Perl
X<control flow>
...
the other keywords.
--:
L<C<break>|/break> доступна только если вы включили эксперементальную функцию
L<C<"switch"> feature|feature/The 'switch' feature> или если она предваряется
префиксом C<CORE::>. L<C<"switch"> feature|feature/The 'switch' feature>
активирует конструкции C<default>, C<given> и C<when>, которые документированы
в L<perlsyn/"Конструкция switch">.
L<C<"switch"> feature|feature/The 'switch' feature> активирована автоматически
объявлением C<use v5.10> (или выше) в текущей области видимости.  В Perl v5.14
и ранее, L<C<continue>|/continue BLOCK> требовал
L<C<"switch"> feature|feature/The 'switch' feature>, как и другие ключевые
слова.
--:notes
Найти более подходящий перевод для Switch Statements
Switch Statements - Конструкция switch
...
current scope.
--:
L<C<evalbytes>|/evalbytes EXPR> доступна только с
L<C<"evalbytes"> feature|feature/The 'unicode_eval' and 'evalbytes' features>
(см. L<feature>) или если она предваряется C<CORE::>.  L<C<__SUB__>|/__SUB__>
доступна только с
L<C<"current_sub"> feature|feature/The 'current_sub' feature>
или если она предваряется C<CORE::>. Обе
функции L<C<"evalbytes">|feature/Функции 'unicode_eval' и 'evalbytes'> и
L<C<"current_sub">|feature/Функция 'current_sub'> включены автоматически
при объявлении C<use v5.16>(или выше) в текущей области видимости.

=item Keywords related to scoping
--:old
=item Ключевые слова для управления областью видимости
--:g
=item Ключевые слова, связанные с
--:
=item Ключевые слова, связанные с областью видимости
...
scope.
--:
L<C<state>|/state VARLIST> доступна только если
L<C<"state"> feature|feature/конструкция 'state'> активирована или
если она предваряется C<CORE::>.
L<Конструкция C<"state">|feature/Конструкция 'state'>  автоматически
активируется объявлением C<use v5.10>(или выше)  в текущей области
видимости.

=item Miscellaneous functions
--:g
=item Различные функции
--:
=item Различные функции
...
--:old
=item Функции для работы с процессами и группами процессов
--:g
=item Функции для процессов и групп процессов
X<process> X<pid> X<process id>
--:
=item Функции для работы с процессами и группами процессов
...
X<module>
--:g
=item Ключевые слова, связанные с модулями Perl
X<module>
--:
=item Ключевые слова, связанные с модулями Perl
X<module>
...
--:old
=item Функции для реализации классов и объектов
X<object> X<class> X<package>
--:g
=item Ключевые слова, связанные с классами и объектно-ориентированными
X<object> X<class> X<package>
--:
=item Ключевые слова, имеющие отношение к классам и ООП
X<object> X<class> X<package>
...
X<socket> X<sock>
--:g
=item Низкоуровневые функции сокетов
X<socket> X<sock>
--:
=item Низкоуровневые функции сокетов
X<socket> X<sock>
...
--:old
=item Функции для межпроцессного взаимодействия в стиле System V
X<IPC> X<System V> X<semaphore> X<shared memory> X<memory> X<message>
--:g
=item Функции межпроцессного взаимодействия системы V
X<IPC> X<System V> X<semaphore> X<shared memory> X<memory> X<message>
--:
=item Функции межпроцессного взаимодействия System V
X<IPC> X<System V> X<semaphore> X<shared memory> X<memory> X<message>
...
--:old
=item Получение информации о пользователях и группах
X<user> X<group> X<password> X<uid> X<gid>  X<passwd> X</etc/passwd>
--:g
=item Получение информации о пользователе и группе
X<user> X<group> X<password> X<uid> X<gid>  X<passwd> X</etc/passwd>
--:
=item Получение информации о пользователе и группе
X<user> X<group> X<password> X<uid> X<gid>  X<passwd> X</etc/passwd>
...
X<network> X<protocol> X<host> X<hostname> X<IP> X<address> X<service>
--:g
=item Получение информации о сети
X<network> X<protocol> X<host> X<hostname> X<IP> X<address> X<service>
--:
=item Получение информации о сети
X<network> X<protocol> X<host> X<hostname> X<IP> X<address> X<service>
...
X<time> X<date>
--:old
=item Функции для работы со временем 
X<time> X<date>
--:g
=item Временные функции 
X<time> X<date>
--:yandex
=item Функции, связанные со временем 
X<time> X<date>
--:
=item Функции даты и времени
X<time> X<date>
...
--:g
=item Неиспользуемые ключевые слова
--:
=item Прочие ключевые слова
...
by this are:
--:old
Perl родился в системе Unix, поэтому он может использовать все общие для Unix
системные вызовы. В окружении отличном от Unix, функционал некоторых системных
вызовов может оказаться недоступным, или функционал может немного различаться.
К таким функция относятся:
--:g
Perl родился в Unix и поэтому может получить доступ ко всем общим Unix
системные вызовы. В средах, отличных от Unix, функциональность некоторых
Системные вызовы Unix могут быть недоступны или сведения о доступных
функциональность может незначительно отличаться. Функции Perl затронуты
это:
--:
Perl родился в системе Unix и поэтому может получить доступ ко всем общим для
Unix системным вызовам. В средах, отличных от Unix, функциональность некоторых
системных вызовов может быть недоступна или незначительно отличаться.
Функции Perl, затронутые здесь:
...
принимает один аргумент, это может быть имя файла, файловый дескриптор, либо
дескриптор каталога, и проверяет соответствующий файл на соответствие условию.
Ели аргумент не указан, проверяет $_ , кроме работы с ключом -t, который
использует STDIN. Если не оговорено иное, возвращает 1 для успешной проверки и
'' для не успешной, либо неопределенное значение если файл не существует.
Несмотря на забавные имена, приоритет у файловых проверок такой же как и у
прочих именованных унарных операторов. Оператор может быть один из:
--:g
Проверка файла, где X - одна из букв, перечисленных ниже. Этот унарный
оператор принимает один аргумент, либо имя файла, дескриптор файла, либо dirhandle,
и проверяет связанный файл, чтобы узнать, правда ли что-то об этом. Если
аргумент опущен, тесты $ _, за исключением -t, который
тесты STDIN. Если не оговорено иное, оно возвращает 1 для истины и
'' за ложь. Если файл не существует или не может быть проверен,
возвращает undef EXPR и устанавливает $! (Ошибка).
Несмотря на смешные имена, приоритет такой же, как и любой другой унарный
оператор. Оператор может быть любым:
--:
Проверка файла, где X - одна из букв, перечисленных ниже. Этот унарный
оператор принимает один аргумент - имя файла, дескриптор файла или  дескриптор
каталога, и проверяет связанный файл на соответствие критерию. Если аргумент
не указан, проверяет L<C<$_>|perlvar/$_>, за исключением C<-t>, который проверяет
STDIN.  Если не оговорено иное, возвращает C<1> за истину и C<''> за ложь.
Если файл не существует или не может быть проверен, возвращает L<C<undef>|/undef EXPR> 
и устанавливает L<C<$!>|perlvar/$!> (errno).  Несмотря на забавные имена, 
приоритет такой же, как и у любого другого унарного оператора. Оператор может 
быть любым из указанных ниже:
...
--:g
	-r Файл доступен для чтения с помощью uid / gid.
	-w Файл доступен для записи эффективным uid / gid.
	-x Файл является исполняемым с помощью эффективного uid / gid.
	-o Файл принадлежит эффективному uid.
--:note
https://www.multitran.ru/c/m.exe?a=110&t=2522448_1_2&sc=138
как вариант, перевод: 
effective uid/gid -- действующему пользователю
--:
    -r  Файл доступен для чтения действующему uid/gid
    -w  Файл доступен для записи действующему uid/gid
    -x  Файл доступен для выполнения действующему uid/gid
    -o  Файл принадлежит действующему uid
...
--:g
	-R Файл читается с помощью реального uid / gid.
	-W Файл доступен для записи с помощью реального uid / gid.
	-X Файл исполняется с помощью real uid / gid.
	-O Файл принадлежит реальному uid.
--:
    -R  Файл доступен для чтения реальному uid/gid.
    -W  Файл доступен для записи реальному uid/gid.
    -X  Файл доступен для выполнения реальному uid/gid.
    -O  Файл принадлежит реальному uid.
...
--:g
	-e Файл существует.
	-z Файл имеет нулевой размер (пуст).
...
--:g
	-f Файл - это простой файл.
	-d Файл - это каталог.
	-l Файл является символической ссылкой (false, если символические ссылки не являются
	    поддерживается файловой системой).
	-p Файл - это именованный канал (FIFO), или Filehandle - это канал.
	-S Файл - это сокет.
	-b Файл - это специальный файл блока.
	-c Файл является специальным файлом символов.
	-t File Handle открывается в tty.
--:
	-f  Файл является обычным файлом.
	-d  Файл является каталогом.
	-l  Файл является символической ссылкой (false, если символические ссылки не поддерживаются файловой системой).
    -p  Файл или файловый дескриптор является именованным каналом (FIFO).
    -S  Файл является сокетом.
    -b  Файл является блочным специальным файлом.
	-c  Файл является символьным специальным файлом.
    -t  Файловый дескриптор открыт в текущем tty.
--:note
...
http://citforum.ru/operating_systems/unix/glava_14.shtml
Различаются два типа специальных файлов - блочные и символьные (подробности см.
в разделе 3.3). Блочные специальные файлы ассоциируются с такими внешними
устройствами, обмен с которыми производится блоками байтов данных, размером
512, 1024, 4096 или 8192 байтов
--note
https://www.google.ru/search?q=file+handle+vs+file+descriptor&oq=file+handle+vs+&aqs=chrome.1.69i57j0l3.5215j0j1&sourceid=chrome&ie=UTF-8
file handle vs file deskriptor
--note
https://www.multitran.ru/c/m.exe?l1=1&l2=2&s=file+handle
...
todo: разобраться в отличиях file handle - file descriptor
...
--:g
	-u Файл имеет бит setuid.
	-g Файл имеет установленный бит setgid.
	-k Файл имеет липкий бит.
--:
	-u  Для файла установлен бит setuid
	-g  Для файла установлен бит setgid
	-k  Для файла установлен "липкий бит" (sticky bit)
...
--:g
	-T Файл представляет собой текстовый файл ASCII или UTF-8 (эвристическое предположение).
	-B Файл является «двоичным» файлом (напротив -T).
--:
	-T Файл является текстовым в формате ASCII или UTF-8 (эвристическое предположение).
	-B Файл является "двоичным"(бинарным) файлом (в противоположность -T).
...
--:g
	-M  Время начала скрипта минус время изменения файла, в днях.
	-А  То же самое для времени доступа.
	-C  То же самое для времени изменения inode (Unix, может отличаться для других
	платформы)
--:
    -M  Время, прошедшее с момента изменения файла до запуска сценария, в днях.
    -A  Время, прошедшее с момента последнего обращения к файлу 
	до запуска сценария, в днях.
    -C  Время, прошедшее с момента изменения индексного дескриптора(inode) 
	до запуска сценария, в днях(для Unix, для других платформ значение может быть иным).
...
--:
    while (<>) {
        chomp;
        next unless -f $_;  # пропустить специальные файлы
        #...
    }
...
--:old
Обратите внимание, что -s/a/b/ не производит подстановки с отрицанием. Однако
-exp($foo) работает, как должно: только одиночные буквы после минуса трактуются
как проверка файла.
--:g
Заметим, что C<-s/a/b/> не выполняет отрицательную подстановку. поговорка
C<-exp($foo)> все еще работает, как ожидалось, однако: только отдельные буквы
после минуса интерпретируются как тесты файлов.
--:note
google переводит фразу:
	"do a negated substitution" - "делать отрицательную замену"
в мультитран примеры использования слова negated:
	https://www.multitran.ru/c/m.exe?a=3&s=negated%20&sc=138&l1=1&l2=2
можно сделать вывод, что:
	negated substitution => замена с отрицанием
--:
Заметим, что C<-s/a/b/> не выполняет подстановку с отрицанием. Выражение
C<-exp($foo)> работает как и ожидается, только отдельные буквы
после минуса интерпретируются как проверки файлов.
...
course):
--:old
Эти операторы освобождены от "правила похожести на функцию", о котором
говорилось выше. То есть, открывающие скобки после оператора не влияют на то,
как много последующего кода образуют аргумент. Вставьте открывающуюся скобку
перед оператором, чтобы отделить его от кода, который следует за ним (конечно,
это относится только к операторам с более высоким приоритетом, чем унарные
операторы):
--:g
Эти операторы освобождаются от описанного выше правила «выглядит как правило
функции» выше. То есть, открывающая скобка после того, как оператор не влияет
сколько из следующего кода составляет аргумент. Положите отверстие скобки перед
оператором, чтобы отделить его от следующего кода (это применяется только к
операторам с более высоким приоритетом, чем унарные операторы,
курс):
--:
Эти операторы освобождаются от описанного выше правила "выглядит как функция".
То есть, открывающая скобка после оператора не влияет на то, какая часть
последующего кода составляет аргумент. Вставьте открывающую скобку перед
оператором, чтобы отделить его от следующего кода (конечно, это применяется
только к операторам с более высоким приоритетом, чем унарные операторы)
--:note
todo: перфразировать текст:
<<какая часть последующего кода составляет аргумент.>>
варианты:
<<какая часть последующего кода относится к аргументам.>>

    -s($file) + 1024   # probably wrong; same as -s($file + 1024)
    (-s $file) + 1024  # correct
--:
    -s($file) + 1024   # вероятно, ошибка; то же самое, что -s($file + 1024)
    (-s $file) + 1024  # корректно
...
conditions.
--:old
Интерпретация операторов прав доступа -r , -R , -w , -W , -x и -X основывается
исключительно на свойствах файла и пользовательском и групповом идентификаторах
пользователя (uids, gids). Могут быть другие причины, по которым не удается
читать, записывать или выполнять файл, например, управление доступом сетевых
файловых систем, ACL (списки управления доступом), файловые системы только для
чтения, и нераспознанные форматы исполняемого файла. Заметим, что использование
вышеупомянутых шести операторов для тестирования файла, может привести к ошибке
в случае, если в это время над файлом производятся какие либо операциии, что
может вызвать зависание, состояние гонки и прочие неприятности.
--:g
Интерпретация операторов разрешения файлов C <-r>, C <-R>, C <-w>, C <-W>, C
<-x>, а C <-X> по умолчанию основывается исключительно на режиме файла и uids и
gid пользователя. Могут быть и другие причины, по которым вы не можете читать,
писать или выполнять файл: for пример управления доступом к сетевой файловой
системе, списки управления доступом (списки управления доступом), файловые
системы только для чтения и непризнанные исполняемые форматы. Заметка что
использование этих шести конкретных операторов для проверки того, возможно,
является ошибкой, поскольку она может быть открытой для гонки условия.
--:
Интерпретация операторов прав доступа C<-r>, C<-R>, C<-w>, C<-W>, C<-x>, and
C<-X> по умолчанию основывается исключительно на режиме файла, uids и
gid пользователя. Могут быть и другие причины, по которым не удается
читать, писать или выполнять файл: например это управление доступом 
к сетевой файловой системе, ACL (списки управления доступом), файловые
системы только для чтения и нераспознанные исполняемые форматы. Заметим, что
что использование вышеупомянутых шести операторов для проверки файлов,
возможно, является шибкой, поскольку это может привести к состоянию гонки.
--:note
file permission -- права доступа
file permission operators -- операторы прав доступа (или операторы проверки
прав доступа к файлу)
mode of the file -- режим файла
unrecognized executable formats -- нераспознанные исполняемые форматы
(нераспознанные форматы исполняемых файлов)
--:note
Фраза "it may be open to race conditions" может быть переведена так:
это может привести к ошибке в случае, если в это время над файлом производятся
какие либо операциии, что может вызвать зависание, состояние гонки и прочие
неприятности.
...
something else.
--:old
Также отметим, что для суперпользователя на локальной файловой системе,
проверки -r , -R , -w и -W всегда возвращают 1, а -x и -X возвращают 1, если
установлен хоть один бит исполнения. Поэтому в сценариях, выполняемых
суперпользователем, может потребоваться выполнение stat(), чтобы определить
действительный режим файла, либо временно установить другой идентификатор
пользователя(анг. effective uid).
--g
Также обратите внимание, что для суперпользователя в локальных файловых
системах, -r, -R, -w и -W всегда возвращают 1, а -x и -X возвращают 1 если
какой-либо бит выполнения установлен в режиме. Скрипты, выполняемые
суперпользователем может, таким образом, нужно сделать stat FILEHANDLE, чтобы
определить фактический режим файла или временно установить эффективный uid для
что-то другое.
--:
Также обратите внимание, что для суперпользователя в локальных файловых
системах, тесты C<-r>, C<-R>, C<-w>, и C<-W> всегда возвращают 1, 
а C<-x> и C<-X> возвращают 1, если в режиме установлен любой бит исполнения.
Таким образом, скриптам, выполняемым суперпользователем, нужно выполнить
L<C<stat>|/stat FILEHANDLE> для определения действительного режима файла,
либо временно установить эффективный uid в другое значение.
--:note
неясно как перевести подфразу "in the mode" в фразе
"if any execute bit is set in the mode". Если перевести как,
"если установлен хоть один бит исполнения В РЕЖИМЕ", звучит не очень.
upd:
перевел как "если в режиме установлен любой бит исполнения"
...
--:old
Если используется ACL, полезна прагма filetest , которая обеспечивает более
точные результаты чем stat(). При вызове use filetest 'access' вышеупомянутые
операторы работают с использованием access(2) или аналогичных системных
вызовов. Отметим также, что под действием этой прагмы операторы -x и -X могут
возвращать истинное значение, даже если биты прав на выполнение не установлены
( или не выставлены какие либо дополнительные права ACL). Эта странность
происходит в основе реализации низкоуровневых системных вызовов. Отметим также,
что в связи с реализацией use filetest 'access', там, где распространяется её
действие, специальный файловый дескриптор _ не будет кэшировать результаты
выполнения операторов проверки файлов. За более подробной информацией
обратитесь к документации прагмы filetest.
--:g
Если вы используете ACL, существует прагма L <C <filetest> | filetest>
которые могут давать более точные результаты, чем голые
L <C <stat> | / stat FILEHANDLE> бит режима.
Когда в C <используйте filetest 'access'>, вышеупомянутые файловые тесты
проверить, можно ли предоставить разрешение (не), используя L <access (2)>
семейство системных вызовов. Также обратите внимание, что тесты C <-x> и C <-X> могут
под этой прагмой возвращается true, даже если нет разрешения на выполнение
бит (или ACL дополнительных разрешений на выполнение). Эта странность
из-за определений базовых системных вызовов. Заметим также, что из-за
реализация C <use filetest 'access'>, C <_> special
filehandle не будет кэшировать результаты тестов файлов, когда эта прагма
в результате. Прочитайте документацию для L <C <filetest> | filetest>
прагма для получения дополнительной информации.
--:
Если используется ACL, существует прагма L<C<filetest>|filetest>, которая может
выдавать более точные результаты, чем простой L<C<stat>|/stat FILEHANDLE> режим
битов. Когда под действием C<use filetest 'access'>,  вышеупомянутые тесты
файлов проверяют, даны ли полномочия используя семейство системных вызовов
L<access(2)>. Также обратите внимание, что тесты C<-x> и C<-X> могут под этой
прагмой возвращать true, даже если не установлены биты разрешения на выполнение
(или любые дополнительные права ACL на выполнение). Эта странность из за
определений базовых системных вызовов. Заметим также, что из за реализации
C<use filetest 'access'>, специальный дескриптоа файла C<_> не будет кэшировать
результаты тестов файлов когда действует эта прагма.  Для получения
дополнительной информации прочитайте документацию прагмы
L<C<filetest>|filetest> .
--:note
todo: доработать первод в данном контексте 'mode bits'
todo: доработать перевод filetests.
--:note
перевод фразы "whether the permission can be granted" от гугла
"разрешено ли разрешение", перевод с использованием multitran - 
"даны ли полномочия или нет".
--:note
Access Control List или ACL — список управления доступом, который определяет,
кто или что может получать доступ к объекту (программе, процессу или файлу), и
какие именно операции разрешено или запрещено выполнять субъекту (пользователю,
...
--:old
Ключи -T и -b работают следующим образом. Участок файла примерно в объеме
первого блока исследуется на наличие необычных символов, таких как коды
управления или байты с установленным старшим битом (не похожие на UTF-8). Если
необычных символов слишком много (больше 30%), то файл считается двоичным(-B),
иначе это текстовый(-T) файл. Также, любой файл считается двоичным, если он
содержит в первом блоке нулевой байт ( символ ASCII NUL(\0) ). Если -T или -B
применяется к дескриптору файла, то исследуется не первый блок файла, а текущий
буфер ввода ( стандартный I/O или "stdio"). Оба оператора -T и -B возвращают
истину для пустого файла, или файла, указатель которого находится в EOF - конце
файла, если проверяется дескриптор. Поскольку нужно прочесть файл, чтобы
выполнить проверку -T, в большинстве случаев лучше сначала сделать проверку с
ключом -f, например next unless -f $file && -T $file.
--:g
Тесты -T и -B работают следующим образом. Первый блок или около того файл
проверяется, является ли он действительным UTF-8, который включает в себя
не-ASCII персонажи. Если это так, это файл -T. В противном случае эта же часть
файл рассматривается для нечетных символов, таких как странные управляющие коды
или символы с высоким набором бит. Если более трети символы странные, это
-B-файл; в противном случае это -T-файл.  Кроме того, любой файл, содержащий
нулевой байт в исследуемой части, считается двоичным файлом. (Если выполняется
в рамках L <S <use locale> | perllocale>, который включает LC_CTYPE, нечетные
символы все, что не является пригодным для печати или пространством в текущей
локали.) Если -T или -B используется в дескрипторе файла, текущий IO-буфер
рассмотрел а не первый блок. Оба -T и -B возвращают true на пустой файл или
файл в EOF при проверке дескриптора файла. Потому что вы должны прочитайте
файл, чтобы выполнить тест -T, в большинстве случаев вы хотите использовать -f
перед файлом сначала, как в C <next, если только -f $ file && -T $ file>.
--:
Тесты C<-T> и C<-B> работают следующим образом. Примерно первый блок файла
исследуется, чтобы определить, является ли он UTF-8, который включает в себя
не-ASCII символы. Если это так, это файл C<-T>. В противном случае эта же
часть файла исследуется на наличие необычных символов, таких как управляющие коды 
или символы с установленным старшим битом. Если более, чем треть символов
составляют необычные символы, это C<-B> файл, в ином случае это C<-T> файл.
Кроме того, любой файл, содержащий нулевой байт в исследуемой части, считается
двоичным файлом. (Если выполняется в области видимости L<S<use locale>|perllocale>,
который включает C<LC_CTYPE>, к необычным символам относятся все непечатаемые
и не пробельные символы в текущей локали).
--:notes
понятие "control codes", которое переводится, как "управляющие коды"
означает то же самое, что и "управляющие символы"
...
control codes - http://jkorpela.fi/chars/c0.html
...
Example:
--:old
Если какому-либо из проверок файлов (или операторам stat или lstat операторам)
передается специальный дескриптор файла, состоящий из "_" - одного символа
подчеркивания, то используется структура stat предыдущей проверки файла (или
оператора stat ), что позволяет избежать нового системного вызова. (Это не
работает для оператора -t, и нужно помнить, что lstat и -l сохраняют значения в
структуре stat, относящиеся к символической ссылке, а не к реальному файлу.)
(Аналогично, если stat буфер был заполнен вызовом lstat, -T и -B сбросят его с
результатом stat _). Например:
--:g
Если какой-либо из тестов файла (или L <C <stat> | / stat FILEHANDLE> или
L <C <lstat> | / lstat FILEHANDLE>) предоставляется специальная дескриптор файла
состоящий из одиночной подчеркивания, то статическая структура
используется предыдущий файл test (или L <C <stat> | / stat FILEHANDLE>),
сохранение системного вызова. (Это не работает с C <-t>, и вам нужно
помните, что L <C <lstat> | / lstat FILEHANDLE> и C <-l> оставляют значения в
структура stat для символической ссылки, а не реальный файл.) (Кроме того, если
буфер статистики был заполнен вызовом L <C <lstat> | / lstat FILEHANDLE>,
C <-T> и C <-B> сбрасывают его с результатами C <stat_>).
Пример:
--:
Если какому либо из тестов файлов (или любому из операторов 
L<C<stat>|/stat FILEHANDLE> или L<C<lstat>|/lstat FILEHANDLE>) предоставляется
специальный дескриптор файла, состоящий из одиночного символаподчеркивания, то
используется структура stat предыдущего теста файла (или оператора 
L<C<stat>|/stat FILEHANDLE>), что позволяет избежать нового системного вызова.
Это не работает с C<-t> и нужно помнить, что L<C<lstat>|/lstat FILEHANDLE> и 
C<-l> сохраняют в структуре stat значения, относящиеся к символической 
ссылке, а не к реальному файлу.
...
--:old
Начиная с версии Perl 5.9.1, как форма синтаксического сахара может
использоваться стек операторов тестирования файла. Таким образом, -f -w -x
$file эквивалентно -x $file && -w _ && -f _. (Но если использовать возвращаемое
значение -f $file в качестве аргумента для другого оператора, никакой особой
магии не произойдет.)
--:g
Начиная с Perl 5.10.0, как форма чисто синтаксического сахара, вы можете складывать файл
так что C <-f -w -x $ file> эквивалентен
C <-x $ file && -w _ && -f _>. (Это всего лишь модный синтаксис: если вы используете
возвращаемое значение C <-f $ file> в качестве аргумента для другого filetest
оператора, никакой особой магии не произойдет.)
--:
Начиная с Perl 5.10.0, для удобства может использоваться стек операторов
тестирования файла таким способом C<-f -w -x $file>, что эквивалентно
C<-x $file && -w _ && -f _>.(Это лишь причудливый синтаксис: если использовать
возвращаемое значение C<-f $file> в аргументе для другого оператора тестирования
файла, никакой особой магии не произойдет.)

Portability issues: L<perlport/-X>.
--:
Вопросы переносимости: L<perlport/-X>.
...
--:g
Чтобы избежать путаницы потенциальных пользователей вашего кода с таинственными
синтаксическими ошибками, поставьте что-то вроде этого в верхней части вашего
скрипта:
--:
Чтобы не сбивать с толку потенциальных пользователей вашего кода загадочными
синтаксическими ошибками, добавьте что-то вроде этого в начало скрипта:
--:note
видимо имеется в виду, что если используется старый интерпретатор, сообщения об
ошибках может не выводиться над конструкцией стека операторов проверки файлов,
с другой стороны объявить минимальную версию perl.

    use 5.010;  # можно использовать стек операторов проверки файлов.
...
--:g
Возвращает абсолютное значение его аргумента. Если VALUE опущено,
используется L<C<$_>|perlvar/$_>.
--:
Возвращает абсолютное значение своего аргумента. Если VALUE опущено, 
используется L<C<$_>|perlvar/$_>.
...
--:old
Принимает входящее соединение на сокет, так же как и системный вызов accept(2).
Возвращает упакованный адрес при успешном соединении и false в противном
случае. См. примеры в спецификации .
--:g
Принимает входящий сокет, так же, как L <accept (2)>. Возвращает упакованный
адрес, если он преуспел, иначе false. См. Пример в L <perlipc / "Sockets:
Client / Server Communication">.
--:
Принимает входящее соединение на сокет, так же как accept(2). Возвращает
упакованный адрес при успешном соединении, в ином случае ложное значение.
См. пример в L<perlipc/"Sockets: Client/Server Communication">.

On systems that support a close-on-exec flag on files, the flag will
be set for the newly opened file descriptor, as determined by the
value of L<C<$^F>|perlvar/$^F>.  See L<perlvar/$^F>.
--:old
В системах, поддерживающих флаг закрытия файлов при выполнении exec
(close-on-exec flag on filesу),этот флаг будет установлен для вновь открытого
файлового дескриптора, в зависимости от значения $^F. См. perlvar.
--:g
В системах, поддерживающих флаг close-on-exec в файлах, флаг будет установлен
для вновь открытого файлового дескриптора, как определено значением L <C <$ ^
F> | perlvar / $ ^ F>. См. L <perlvar / $ ^ F>. 
--:
В системах, поддерживающих флаг close-on-exec дескрипторов файлов, флаг будет
установлен для дескрипторов вновь открытых файлов, как определено значением
L<C<$^F>|perlvar/$^F>. См. L<perlvar/$^F>.
--:note
понятие close-on-exec описывается здесь: http://wm-help.net/lib/b/book/2075737573/201
close-on-exec - флаг закрытия файлов при выполнении exec.
...
--:old
Подготавливает сигнал SIGALRM, который будет передан текущему процессу по
истечении указанного количества секунд. Если SECONDS не указано, используется
значение из $_. (на некоторых машинах, к сожалению, прошедшее время может быть
меньше или больше указанного на одну секунду, в зависимости от того, как
считаются секунды. Из-за этого планировщик процессов может отложить доставку
сигнала на некоторое время).
--:g
Устанавливает, чтобы SIGALRM доставлялся в этот процесс после истечения
заданного количества секунд разгона. Если SECONDS не указывается, используется
значение, хранящееся в L <C <$ _> | perlvar / $ _>. (На некоторых машинах, к
сожалению, прошедшее время может быть на одну секунду меньше или больше, чем вы
указали из-за того, как подсчитываются секунды, а планирование процесса может
еще более замедлить доставку сигнала.)
--:
Определяет заданное число секунд, после истечения которого текущему процессу
посылается сигнал SIGALRM. Если значение SECONDS не задано, используется
значение переменной L<C<$_>|perlvar/$_>.(На некоторых машинах, к сожалению,
прошедшее время может быть меньше или больше до одной секунды чем было указано,
в зависимости от того, как подсчитываются секунды, а планировщик процессов может
задержать доставку сигнала еще больше.)
--:note
"after the specified number of wallclock seconds has elapsed" = "после истечения
заданного количества секунд"
--:note
"Arranges to have a SIGALRM delivered to this process after the specified number
of wallclock seconds has elapsed." = Определяет так, чтобы SIGALRM был
доставлен этому процессу после истечения заданного числа секунд.
...
--:old
Одновременно может быть активен только один таймер. Каждое новое обращение к
функции отключает предыдущий таймер, таким образом, значение SECONDS, равное 0,
может использоваться для отключения предыдущего таймера без запуска нового.
Возвращаемое значение представляет остаток времени предыдущего таймера.
--:g
Только один таймер может рассчитывать сразу. Каждый вызов отключает предыдущий
таймер, а аргумент C <0> может быть подан для отмены предыдущего таймера без
запуска нового. Возвращаемое значение - это время, оставшееся на предыдущем
таймере.
--:
Одновременно, можно установить только один таймер. Каждый новый вызов функции
отключает предыдущий таймер, а аргумент C<0> может быть подан для отмены
предыдущего таймера без запуска нового. Возвращаемое значение представляет
остаток времени предыдущего таймера.
...
--:old
Чтобы задать время подачи сигналов с большей точностью, чем одна секунда, можно
применить функцию ualarm(), которую предоставляет модуль Time::HiRes (можно
найти на CPAN, начиная с Perl 5.8 является частью дистрибутива). Также можно
использовать 4х-аргументную версию select(), оставляя первые три аргумента
неопределенными. Еще один вариант задать миллисекунды предоставляет интерфейс
syscall для обращения к setitimer(2), если система поддерживает его. См.
perlfaq8 подробнее.
--:g
Для задержек более тонкой детализации, чем одна секунда, модуль L <Time ::
HiRes> (из CPAN и начиная с Perl 5.8 часть стандартного дистрибутива)
предоставляет L <C <ualarm> | Time :: HiRes / ualarm ($ useconds [, $
interval_useconds])>. Вы также можете использовать четырехпортовую версию
Perl L <C <select> | / select RBITS, WBITS, EBITS, TIMEOUT>, оставив первые
три аргумента неопределенными, или вы можете использовать L <C <syscall> |
/ syscall NUMBER, LIST> для доступа к L <setitimer (2)>, если ваша система
поддерживает его. Подробнее см. L <perlfaq8>.
--:
Для задержек с градацией меньше секунды, модуль L<Time::HiRes>(из CPAN и
начиная с Perl 5.8 является частью стандартного дистрибутива) предоставляет 
L<C<ualarm>|Time::HiRes/ualarm ( $useconds [, $interval_useconds ] )>.
Можно также использовать функцию L<C<select>|/select RBITS,WBITS,EBITS,TIMEOUT>
с четырьмя аргументами, оставив первые три аргумента неопределенными, или
вы могли бы использовать интерфейс L<C<syscall>|/syscall NUMBER, LIST> 
для доступа к L<setitimer(2)> если система поддерживает его. См. подробнее
в L<perlfaq8>.
...
--:old
Обычно смешивание вызовов alarm и sleep является ошибкой, потому что в вашей
системе функция sleep может быть внутренне реализована с использованием
механизма системного вызова alarm(2).
--:g
Обычно бывает ошибкой смешивать вызовы L <C <alarm> | / alarm SECONDS> и L <C
<sleep> | / sleep EXPR>, потому что L <C <sleep> | / sleep EXPR> может быть
внутренне реализован на вашем система с L <C <alarm> | / alarm SECONDS>.
--:
Обычно бывает ошибкой смешивать вызовы L<C<alarm>|/alarm SECONDS> и
L<C<sleep>|/sleep EXPR>, потому что в вашей системе функция 
L<C<sleep>|/sleep EXPR> может быть внутренне реализована с использованием
L<C<alarm>|/alarm SECONDS>.
...
--:old
Если вы планируете использовать alarm для формирования тайм-аута системного
вызова, необходимо использовать пару eval/die. Не стоит полагаться на то, что,
в случае, если системный вызов не отработает должным образом, переменная $!
будет содержать значение EINTR, поскольку на некоторых операционных системах,
Perl устанавливает обработчики сигналов таким образом, чтобы они перезапускали
системные вызовы. А вот связка eval/die работает всегда, возможные сложности
описаны в perlipc/"Сигналы".
--:g
Если вы хотите использовать <будильник> для выключения системного вызова, вам
нужно использовать пару <eval>. Вы не можете полагаться на тревогу, вызвав
отказ системного вызова с помощью $! установлен на C <EINTR>, потому что Perl
настраивает обработчики сигналов для перезапуска системных вызовов в некоторых
системах. Использование <eval> всегда работает, по модулю предостережений,
указанных в L <perlipc / "Signals">.
--:
Если вы хотите использовать L<C<alarm>|/alarm SECONDS> для прерывания системного
вызова, задействуйте связку L<C<eval>|/eval EXPR>/L<C<die>|/die LIST>.  Не стоит
полагаться на то, что alarm приводит к отказу от системного вызова с
установкой значения C<EINTR> в переменной L<C<$!>|perlvar/$!>, потому что
Perl настраивает обработчики сигналов на перезапуск системных вызовов в некоторых
системах.
--:note
restart system calls/перезапуск системных вызовов 
(инфа https://books.google.ru/books?id=0dgxNHCCHOQC&pg=PA181&lpg=PA181&dq=%D0%BF%D0%B5%D1%80%D0%B5%D0%B7%D0%B0%D0%BF%D1%83%D1%81%D0%BA+%D1%81%D0%B8%D1%81%D1%82%D0%B5%D0%BC%D0%BD%D0%BE%D0%B3%D0%BE+%D0%B2%D1%8B%D0%B7%D0%BE%D0%B2%D0%B0&source=bl&ots=uPjqkQB1fm&sig=96DrA33zstQlQDDepjrvAzw4UmU&hl=ru&sa=X&ved=2ahUKEwj_1fvS95HdAhUGESwKHQzFC_EQ6AEwAHoECAEQAQ#v=onepage&q=%D0%BF%D0%B5%D1%80%D0%B5%D0%B7%D0%B0%D0%BF%D1%83%D1%81%D0%BA%20%D1%81%D0%B8%D1%81%D1%82%D0%B5%D0%BC%D0%BD%D0%BE%D0%B3%D0%BE%20%D0%B2%D1%8B%D0%B7%D0%BE%D0%B2%D0%B0&f=false_)
(книга "Разработка приложений в среде Linux, 2-е издание" Глава 12 стр. 181)
--:note
о прерывании сигналов в Perl:
http://qaru.site/questions/5828901/does-join-in-perl-threads-block-sigalrm
..
Ошибки системных вызовов:
https://it.wikireading.ru/34189
..
о EINTR:
https://wm-help.net/lib/b/book/1268775764/53
| Одни из кодов, с которым приходится сталкиваться наиболее часто, особенно в
| функциях ввода-вывода, — это EINTR . Ряд функций, в частности read(),select() и
| sleep(), требует определенного времени на выполнение.  Они называются
| блокирующими, так как выполнение программы приостанавливается до тех пор, пока
| функция не завершится. Но если программа, будучи заблокированной, принимает
| сигнал, функция завершается, не закончив выполнение операции. В данном случае в
| переменную errno записывается значение EINTR. Обычно в подобной ситуации
| следует повторно выполнить системный вызов.
...
--:
Для получения дополнительной информации см. L<perlipc>.

Portability issues: L<perlport/alarm>.
--:
Вопросы переносимости: L<perlport/alarm>.
...
relation:
--:old
Для вычисления тангенсов можно обратиться к к функции Math::Trig::tan либо
использовать известное соотношение:
--:g
Для касательной операции вы можете использовать функцию Math :: Trig :: tan 
или использовать знакомое отношение:
--:
Для вычисления тангенса можно обратиться к к функции
L<C<Math::Trig::tan>|Math::Trig/B<tan>> либо использовать известное
соотношение:
...
--:old
Возвращаемое значение atan2(0,0) определяется реализацией. Обратитесь к
странице руководства atan2(3) за дополнительной информацией.
--:g
Возвращаемое значение для <atan2> определяется реализацией; обратитесь за
помощью к странице <atan2>> manpage для получения дополнительной информации.
--:
Возвращаемое значение C<atan2(0,0)> определяется реализацией; обратитесь
к справочной странице L<atan2(3)> за дополнительной информацией.
--:note
manpage: https://www.multitran.ru/c/m.exe?t=4504785_2_1&s1=MANPAGE

Portability issues: L<perlport/atan2>.
--:
Вопросы переносимости: L<perlport/atan2>.
...
--:old
Прикрепляет сетевой адрес сокету, как это делает bind(2). Возвращает значение
"истина", если это удалось, и "ложь" в противном случае. NAME должно быть
упакованным адресом сокета, имеющим надлежащий тип. См. примеры в
--:g
Привязывает сетевой адрес к сокету, как это делает <bind (2)>. Возвращает true,
если это удалось, false в противном случае. NAME должен быть упакованным
адресом соответствующего типа для сокета. См. Примеры в <perlipc>.
--:
Связывает сетевой адрес с сокетом, как это делает L<bind(2)>. Возвращает
истинное значение в случае успеха, и ложное значение в противном случае.
--:note
page:780
...
L<C<$!>|perlvar/$!> (errno).
--:old
Устанавливает дескриптор FILEHANDLE для чтения или записи в "бинарный" или
"текстовый" режимы в системах, библиотеки времени исполнения которых различают
текстовые и двоичные данные. Если FILEHANDLE представляет собой выражение, то
его значение принимается за имя дескриптора файла. Функция возвращает значение
"истина" в случае успеха, в противном случае возвращает undef, а переменной $!
присваивается сообщение об ошибке.
--:g
Упорядочивает FILEHANDLE для чтения или записи в режиме «двоичный» или
«текстовый» в системах, где библиотеки времени выполнения различают двоичные и
текстовые файлы. Если FILEHANDLE является выражением, значение берется как имя
дескриптора файла. Возвращает true при успешном выполнении, иначе он возвращает
<< undef> и устанавливает <$!> (Errno).
--:
Устанавливает для FILEHANDLE семантику чтения или записи в двоичном или
текстовом режиме в системах, где библиотеки времени выполнения разлючают
двоичные и текстовые файлы. Если FILEHANDLE является выражением, его значение
принимается как имя дескриптора файла. Возвращает истинное значение при успешном
выполнении, иначе возвращает L<C<undef>|/undef EXPR> и устанавливает
L<C<$!>|perlvar/$!> (errno).
--:note
page:781
...
--:old
в некоторых системах (обычно в системах dos- и windows) необходимо использовать
binmode(), когда вы работаете не с текстовым файлом. в целях портативности
будет хорошей практикой всегда использовать ее при необходимости, и никогда не
использовать там, где она не подходит. кроме того, люди могут устанавливать
свои режимы ввода-вывода по умолчанию для utf8-закодированных данных, а не
байтов.
--:g
На некоторых системах (в общем, DOS- и Windows-системах) <binmode> необходим,
когда вы не работаете с текстовым файлом. Для удобства переноски рекомендуется
всегда использовать его, когда это необходимо, и никогда не использовать его,
когда это не подходит. Кроме того, люди могут устанавливать свой I / O по
умолчанию по кодировке UTF8 Unicode, а не по байтам.
--:
На некоторых системах (обычно, DOS- и Windows-системах) необходимо использовать
L<C<binmode>|/binmode FILEHANDLE, LAYER>, когда работа ведется не с текстовым
файлом. В целях переносимости рекомендуется всегда использовать ее, когда это
необходимо, и никогда не использовать, где это не уместно. Кроме того, люди
могут устанавливать свои режимы ввода-вывода по умолчанию в кодировке UTF8
Unicode, а не байтов.
--:note
...
for example.
--:old
Другими словами, независимо от платформы, используйте binmode() над бинарными
данными, например такими, как изображения.
--:g
другими словами: независимо от платформы, используйте c <binmode> для двоичных данных, например изображений.
--:
Другими словами: независимо от платформы, используйте L<C<binmode>|/binmode FILEHANDLE, LAYER>
для двоичных данных, например изображений.

If LAYER is present it is a single string, but may contain multiple
directives.  The directives alter the behaviour of the filehandle.
When LAYER is present, using binmode on a text file makes sense.
--:old
Если указан уровень LAYER, он является одной строкой, но может содержать
несколько директив. Директивы изменяют поведение дескриптора. Когда LAYER
указан, использование binmode над текстовым файлом имеет смысл.
--:g
Если присутствует LAYER, это одна строка, но может содержать несколько
директив. Директивы изменяют поведение дескриптора файла. Когда LAYER
присутствует, использование binmode в текстовом файле имеет смысл.
--:
Если присутствует LAYER, это одна строка, но может содержать несколько
директив. Директивы изменяют поведение дескриптора. Когда LAYER присутствует,
использование binmode над текстовым файлом имеет смысл.
...
--:old
Если LAYER опущен или указан как :raw,то дескриптор файла становится пригодным
для передачи бинарных данных. Это включает в себя отключение возможных
переводов CRLF и маркировки его как байт (в отличие от символов Юникода).
Отметим, что несмотря на то, что может подразумеваться в "Программирование на
Perl" (Камел, 3-е издание) или где-либо еще, :raw не просто обратна :crlf.
Другие уровни, которые могут повлиять на бинарный характер потока также
отключены. См. PerlIO, perlrun и обсуждение переменной среды PERLIO.
--:g
Если LAYER опущен или указан как <: raw, создается файл-дескриптор
подходит для передачи двоичных данных. Это включает отключение CRLF
перевод и обозначение его как байтов (в отличие от символов Unicode).
Обратите внимание, что, несмотря на то, что может подразумеваться в <«Программирование Perl» (
Camel, 3-е издание) или в другом месте, <: raw> is <not> просто обратный <: crlf>.
Другие слои, которые влияют на двоичный характер потока,
<также> отключено. См. <PerlIO>, <perlrun> и обсуждение
Переменная среды PERLIO.
--:
Если LAYER опущен или указан как C<:raw>, дескриптор файла становится пригодным
для передачи двоичных данных. Это включает в себя отключение возможных
переводов CRLF и маркировки его как байтов (в отличие от символов Unicode).
Отметим, что несмотня на то, что может подразумеваться в I<"Программирование на
Perl"> (Camel, 3-е издание) или где либо еще, C<:raw> I<не> просто обратна C<:crlf>.
Другие уровни, которые могут повлиять на двоичный характе потока L<также> отключены.
См. L<PerlIO>, L<perlrun>, и обсуждение переменной среды PERLIO.
--:note
"the filehandle is made suitable for passing binary data"=
"дескриптор файла предназначен для передачи двоичных данных"
...
--:old
Директивы :bytes, :crlf, :utf8 и любые другие директивы в форме :... называются
уровнями(layers) ввода-вывода. Для установки уровней ввода-вывода по умолчанию
можно использовать прагму open. См. open.
--:
Директивы C<:bytes>, C<:crlf>, C<:utf8>, и любые другие директивы в форме
C<:...>, называются уровнями ввода-вывода. Для установки уровней ввода-вывода
по умолчанию можно использовать прагму L<open>.
...
--:old
Параметр LAYER функции binmode() описывается как "Дисциплина" в книге
"Программирование на Perl, 3-го издания. Однако, с тех пор консенсус именования
данной функциональности сместилась с "дисциплина" на "уровень". По этой причине
в документации текущей версии Perl мы используем понятие "уровни", а не
"дисциплины".
--:g
Параметр LAYER функции binmode описан как «DISCIPLINE» в «Программирование на
Perl, 3-е издание». Однако после публикации этой книги, известной многим как
«Camel III», консенсус в отношении названия этой функциональности перешел от
«дисциплины» к «слою». Поэтому вся документация этой версии Perl относится к
«слоям», а не к «дисциплинам». Теперь вернемся к регулярной документации ...
--:
I<Параметр LAYER функции L<C<binmode>|/binmode FILEHANDLE, LAYER> описывается
как "дисциплина" в "Программирование на Perl, 3-го издания. Однако после
публикации этой книги, известной многим как "Camel III", консенсус именования
данной функциональности сместился с "дисциплины" на "уровень".  Поэтому во всей
документации текущей версии Perl используется понятие "уровень" а не "дисциплина".
Теперь вернемся к текущей документации..>
--:note
найти более подходящий вариант перевода:
	"back to the regularly scheduled documentation"
--:todo
Определиться, как называть - "слой" или "уровень". В книге Camel IV русское 
издание на стр 781+ используется понятие "уровень" а также "фильтр"
...
--:old
Чтобы отметить FILEHANDLE как UTF-8, используйте :utf8 или :encoding(UTF-8).
:utf8 только помечает данные как UTF-8 без дополнительной проверки, в то время
как :encoding(UTF-8) проверяет данные на фактическую валидность UTF-8.
Подробности можно найти в PerlIO::encoding.
--:
Чтобы пометить FILEHANDLE как UTF-8, используйте C<:utf8> или C<:encoding(UTF-8)>.
C<:utf8> просто маркирует данные как UTF-8 без дополнительной проверки,
в то время как C<:encoding(UTF-8)> проверяет данные на фактическую валидность
UTF-8.  Более подробную информацию можно найти в L<PerlIO::encoding>.
...
буферизованные данные вывода (и, возможно, ожидающие данные ввода) дескриптора.
Исключением является уровень :encoding, который изменяет для дескриптора
кодировку по умолчанию, см . Уровень :encoding иногда необходимо вызывать в
середине потока и это не сбрасывает поток. :encoding также неявно задействует
уровень :utf8, поскольку внутренне Perl оперирует над utf-8 кодированными
символами.
--:g
В общем случае <binmode> должен вызываться после <open>, но до того, как будет
выполнен какой-либо ввод-вывод для дескриптора файла. Вызов <binmode> обычно
сбрасывает любые ожидающие буферизованные выходные данные (и, возможно,
ожидающие входные данные) на дескриптор. Исключением является слой C <:
encoding>, который изменяет кодировку символов дескриптора по умолчанию. Слой C
<: encoding> иногда нужно вызывать в среднем потоке, и он не сбрасывает поток.
C <: encoding> также неявно помещает поверх себя слой C <: utf8>, потому что
внутри Perl работает с символами Unicode в кодировке UTF8.
--:
В общем случае, L<C<binmode>|/binmode FILEHANDLE, LAYER> должен вызываться
после L<C<open>|/open FILEHANDLE,EXPR> но до того, как будет
выполнен какой-либо ввод-вывод для дескриптора файла.
Вызов L<C<binmode>|/binmode FILEHANDLE, LAYER> обычно сбрасывает любые
ожидающие буферизованные выходные данные (и возможно, ожидающие входные данные)
дескриптора.  Исключением этого является уровень C<:encoding>, который  изменяет
для дескриптора кодировку символов по умолчанию. Иногда бывает необходимость
вызывать уровень C<:encoding> в середине потока и это не сбрасывает поток.
C<:encoding> также неявно задействует уровень C<:utf8>, 
посколько внутренне Perl оперирует символами Unicode в кодировке UTF8.
...
исполнения Perl работают совместно, чтобы дать возможность программисту
использовать один символ (\n) как конец строки, независимо от внешнего
представления. На многих операционных системах, простой текстовый файл
соответствует внутреннему представлению, но на некоторых платформах внешнее
представление \n состоит из более, чем одного символа.
--:g
Операционная система, драйверы устройств, библиотеки C и система времени
исполнения Perl - все это позволяет программисту обрабатывать один символ (C <\
n>) как терминатор строки, независимо от внешнего представления. Во многих
операционных системах нативное представление текстового файла соответствует
внутреннему представлению, но на некоторых платформах внешнее представление C
<\ n> состоит из более чем одного символа.
--:
Операционная система, драйверы устройств, библиотеки C и система времени
выполнения Perl работают совместно, чтобы дать возможность программисту
использовать один символ (C<\n>) как конец строки, независимо от внешнего
представления. На многих операционных системах нативное представление
текстового файла соответствует внутреннему представлению, но на некоторых
платформах внешнее представление C<\n> состоит из более чем одного символа.
...
--:old
Все вариации Unix, Mac OS(старый и новый), и файлы Stream_LF в VMS используют
один символ конца каждой строки во внешнем представлении текста (даже, если это
один символ возврата каретки в старом, pre-Darwein olavors of Mac OS и перевода
строки в Unix и в большинстве VMS файлов). В других системах, подобных OS/2,
Dos и в различных разновидностях MS-Windows, ваша программа видит \n как один
символ \cJ, но что хранится в текстовых файлах является двумя символами \cM\cJ.
Это означает, что, если вы не используете binmode() в этих системах,
последовательности \cM\cJ на диске будут преобразованы в \n на вводе и любые \n
вашей программы будут преобразованы обратно в \cM\cJ на выводе. Это то что вам
нужно для текстовых файлов, но это может быть катастрофическим для двоичных
файлов.
--:g
Во всех вариантах файлов Unix, Mac OS (старых и новых) и Stream_LF в VMS
используется один символ для завершения каждой строки во внешнем представлении
текста (даже если этот единственный символ является символом возврата каретки в
старых версиях Mac, предшествовавших Дарвину) ОС и является LINE FEED для Unix
и большинства файлов VMS). В других системах, таких как OS / 2, DOS и различные
разновидности MS-Windows, ваша программа видит \ n как простой \ cJ, но в
текстовых файлах хранятся два символа \ cM \ cJ. Это означает, что если вы не
используете binmode в этих системах, последовательности \ cM \ cJ на диске
будут преобразованы в \ n на входе, а любые \ n в вашей программе будут
преобразованы обратно в \ cM \ cJ на выходе. Это то, что вы хотите для
текстовых файлов, но это может иметь катастрофические последствия для двоичных
файлов.
--:
Во всех вариациях Unix, Mac OS (старый и новый), и файлах формата Stream_LF в VMS
используется один символ перевода строки во внешнем представлении текста (даже если
этот единственный символ является символом возврата каретки в файлах старых pre-Darwin версиях
Mac, переводом строки в файлах в системе Unix и в большинстве систем VMS).
В других системах, таких как OS/2, DOS, и в различных разновидностях MS-Windows,
ваша программа видит C<\n> как простой C<\cJ>, но то что хранится в текстовых
файлах является двумя символами C<\cM\cJ>.
--:process
and is LINE FEED on Unix and most VMS files
--:note
|---------------------------------------------------------------------------
| https://groups.google.com/forum/#!topic/comp.os.vms/QdGywu7_Vxk
|---------------------------------------------------------------------------
| Gene Homan wrote:
| > Does anyone know what the end of line character is in VMS or, better yet,
| > if there is a utility (preferably that runs on Solaris, Linux, etc) to
| > convert VMS ascii txt files to UNIX format?
| a VMS file in STREAM format uses <CR><LF> as EOL
| a VMS file in STREAM_LF format uses <LF> as EOL
| a VMS file in STREAM_CR format uses <CR> as EOL
| a VMS file in VARIABLE LENGTH, VFC, FIXED LENGTH or UNDEFINED format do
| not use an EOL marker
| 
| The C RTL on VMS interfaces the physical format as a logical <LF>
| terminated
| format for files opened in text mode.
| 
| Text files FTP'ed from VMS to Unix should automatically be converted to
| <LF> format (if FTP'ed in proper text mode).
| 
| Confused ?
| 
| If you try and post some more details about what you have and what the
| problem
| is, then maybe we can give you some better and more specific advice.
--:note
search: vms file format stream-lf
...
что специальные маркеры конца файла будут рассматриваться как часть потока
данных. Для систем из семейства Microsoft это означает, что, если бинарные
данные содержат \cZ и вы не используете binmode(), то подсистема ввода-вывода
эту последовательность принимает за конец файла.
--:g
Другое следствие использования binmode (в некоторых системах) заключается в
том, что специальные маркеры конца файла будут рассматриваться как часть потока
данных. Для систем из семейства Microsoft это означает, что, если ваши двоичные
данные содержат \ cZ, подсистема ввода-вывода будет рассматривать их как конец
файла, если вы не используете binmode.
--:
Другое следствие использования L<C<binmode>|/binmode FILEHANDLE, LAYER> (в
некоторых системах) заключается в том, что специальные маркеры конца файла
будут рассматриваться как часть потока данных.  Для систем из семейства
Microsoft это означает, что, если ваши двоичные данные содержат C<\cZ>,
подсистема ввода-вывода будет рассматривать их как конец файла, если вы не
используете L<C<binmode>|/binmode FILEHANDLE, LAYER>.
...
--:
L<C<binmode>|/binmode FILEHANDLE, LAYER> имеет важное значение не только
для операций
L<C<readline>|/readline EXPR> и L<C<print>|/print FILEHANDLE LIST>,
но также при использовании
L<C<read>|/read FILEHANDLE,SCALAR,LENGTH,OFFSET>,
L<C<seek>|/seek FILEHANDLE,POSITION,WHENCE>,
L<C<sysread>|/sysread FILEHANDLE,SCALAR,LENGTH,OFFSET>,
L<C<syswrite>|/syswrite FILEHANDLE,SCALAR,LENGTH,OFFSET> и
L<C<tell>|/tell FILEHANDLE> (подробнее об этом в L<perlport>). См.
переменные
L<C<$E<sol>>|perlvar/$E<sol>> и L<C<$\>|perlvar/$\> в
L<perlvar> для того, чтобы вручную установить последовательности завершения
строки для ввода и вывода.

Portability issues: L<perlport/binmode>.
--:
Вопросы переносимости: L<perlport/binmode>.
...
--:g
Эта функция сообщает объекту, на который ссылается REF, что теперь это объект в
пакете CLASSNAME. Если CLASSNAME опущено, используется текущий пакет. Поскольку
благословение часто является последним в конструкторе, для удобства оно
возвращает ссылку. Всегда используйте версию с двумя аргументами, если
производный класс может наследовать метод, выполняющий благословение. Смотрите
perlobj для получения дополнительной информации о благословении (и
благословениях) объектов.
--:
Эта функция сообщает объекту, на который указывает ссылка REF, что теперь он
является объектом в пакете CLASSNAME. Если CLASSNAME не задано, используется
текущий пакет. Поскольку L<C<bless>|/bless REF,CLASSNAME> часто является
последним в конструкторе, для удобства оно возвращает ссылку. Всегда
используйте версию с двумя аргументами, если производный класс может
наследовать метод, выполняющий L<C<bless>|/bless REF,CLASSNAME>. Смотрите
L<perlobj> для получения дополнительной информации о создании объектов класса.
...
--:g
Всегда учитывайте благословение объектов в именах CLASSNAME, которые имеют
смешанный регистр. Пространства имен со всеми строчными именами считаются
зарезервированными для прагм Perl. Встроенные типы имеют все прописные имена.
Чтобы избежать путаницы, вы можете также избегать таких имен пакетов.
Убедитесь, что CLASSNAME является истинным значением.
--:
Всегда учитывайте, что "благословение" объектов должно быть в классах
CLASSNAME, имена которых имеют смешанный регистр. Пространства имен со всеми
именами в нижнем регистре считаются зарезервированными для прагм Perl. Все встроенные типы
имеют имена в верхнем регистре. Чтобы избежать путаницы, вы также можете избегать
таких имен пакетов. Убедитесь, что CLASSNAME является истинным значением.

See L<perlmod/"Perl Modules">.
--:
См. L<perlmod/"Perl Modules">.
...
scope.
--:
Поддержка L<C<break>|/break> доступна только при включенном 
L<C<"switch"> feature|feature/The 'switch' feature> или если предварить
его префиксом C<CORE::>.  Свойство
L<C<"switch"> feature|feature/The 'switch' feature> включается автоматически
при объявлении C<use v5.10> (или выше) в текущей области видимости.
...
--:old
Возвращает контекст вызова текущей подпрограммы. В скалярном контексте,
возвращает имя пакета, инициировавшего вызов, если есть инициатор вызова (т.е.,
если мы находимся в подпрограмме, eval или require) и неопределенное значение в
противном случае. В списковом контексте, возвращает:
--:g
Возвращает контекст текущего чистого вызова подпрограммы perl. В скалярном
контексте возвращает имя пакета вызывающего, если есть вызывающий (то есть,
если мы находимся в подпрограмме или <eval> или <require>, в противном случае
неопределенное значение. Caller никогда не возвращает подпрограммы XS, и они
пропускаются. следующий чистый perl sub появится вместо XS sub в возвращаемых
значениях вызывающего абонента.
--:
Возвращает контекст вызова текущей подпрограммы. В скалярном контексте,
возвращает имя пакета, инициировавшего вызов, если есть инициатор вызова (то
есть, если мы находимся в подпрограмме, L<C<eval>|/eval EXPR> или
L<C<require>|/require VERSION>) и неопределенное значение в противном случае.
Функция C<caller> никогда не возвращает подпрограммы XS и они пропускаются.
Следующая подпрограмма на чистом perl появится вместо подпрограммы XS в
возвращаемых значениях функции caller. В списковом контексте, C<caller>
возвращает
--:note
О том, что такое "pure perl" - https://www.perlmonks.org/?node_id=709757
Пока опустил перевод значения pure perl, позже внести корректировки.
(вариант: <Возвращает pure perl контекст вызова текущей подпрограммы.>)
