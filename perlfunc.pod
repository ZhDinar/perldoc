...
Эта функция сообщает объекту, на который ссылается REF, что теперь это объект в
пакете CLASSNAME. Если CLASSNAME опущено, используется текущий пакет. Поскольку
L<C<bless>|/bless REF,CLASSNAME> часто является последним в конструкторе, для
удобства оно возвращает ссылку. Всегда используйте версию с двумя аргументами,
если производный класс может наследовать метод, выполняющий
L<C<bless>|/bless REF,CLASSNAME>. Смотрите L<perlobj> для получения
дополнительной информации о создании объектов класса.
