...
--:
	...
	-Устанавливает, чтобы SIGALRM доставлялся в этот процесс после истечения заданного количества секунд разгона., 
	-Организует иметь SIGALRM доставлены в этот процесс после того, как заданное количество секунд стандартной даты истечения.

	If SECONDS is not specified, the value stored in L<C<$_>|perlvar/$_> is used.
	- Если SECONDS не указывается, используется значение, хранящееся в L <C <$ _> | perlvar / $ _>., 
	- Если СЕКУНД не указан, то значение, которое хранится в L <C <$ _> | perlvar / $ _> используется.

	(On some machines, unfortunately, the elapsed time may be up to one second less
	or more than you specified because of how seconds are counted, and process
	scheduling may delay the delivery of the signal even further.)
	-(На некоторых машинах, к сожалению, прошедшее время может быть на одну секунду
	меньше или больше, чем вы указали из-за того, как подсчитываются секунды, а
	планирование процесса может еще более замедлить доставку сигнала.), 
	-(На некоторых машинах, к сожалению, прошедшее время может быть до одной
	секунды меньше или больше, чем указано, поскольку, как секунды подсчитываются,
	и планирование процесса может задержать доставку сигнала еще больше.)
