...
который включает C<LC_CTYPE>, необычными символами является все, что не является
пригодным для печати или пробелами в текущей локали).
	(If executed within the scope of a L<S<use
	locale>|perllocale> which includes C<LC_CTYPE>, odd characters are
	anything that isn't a printable nor space in the current locale.)
